...
import type { AppSettings, InputDeviceInfo, ThemeChoice } from "../../lib/types";
...
export function SettingsPage({
  ...
  useEffect(() => { void adapter.listInputDevices().then(setDevices).catch(() => setDevices([])); }, [adapter]);
  useEffect(() => {
    applyTheme(theme);
      ...
      <div className="settings-layout">
        <div className="settings-column">
          <section className="settings-group">
            ...
            <label className="setting-row"><span><strong>Mikrofon</strong><small>Urządzenie używane do dyktowania</small></span><select value={settings.inputDevice ?? ""} onChange={(event) => void save({ ...settings, inputDevice: event.target.value || null })}><option value="">Domyślny systemowy</option>{devices.map((device) => <option value={device.id} key={device.id}>{device.name}</option>)}</select></label>
            <label className="setting-row"><span><strong>Skrót klawiszowy</strong><small>{shortcutError || "Działa w każdej aplikacji"}</small></span><input className={shortcutError ? "invalid" : ""} value={settings.shortcut} onChange={(event) => changeShortcut(event.target.value)} onBlur={() => !shortcutError && void save(settings)} aria-invalid={Boolean(shortcutError)} /></label>
          </section>
          <section className="settings-group">
            <div className="group-heading"><h2>Zachowanie</h2></div>
            <label className="setting-row"><span><strong>Wklejaj automatycznie</strong><small>Wstaw tekst do aktywnego okna</small></span><input type="checkbox" checked={settings.autoPaste} aria-label="Wklejaj automatycznie" onChange={(event) => void save({ ...settings, autoPaste: event.target.checked })} /></label>
            <label className="setting-row"><span><strong>Uruchamiaj z systemem</strong><small>Mów będzie gotowe w zasobniku</small></span><input type="checkbox" checked={settings.launchOnLogin} onChange={(event) => void save({ ...settings, launchOnLogin: event.target.checked })} /></label>
            <label className="setting-row"><span><strong>Przechowuj nagrania</strong><small>Automatyczne porządkowanie audio</small></span><select value={settings.retentionDays ?? "forever"} onChange={(event) => void save({ ...settings, retentionDays: event.target.value === "forever" ? null : Number(event.target.value) as 1 | 7 | 30 })}><option value="1">1 dzień</option><option value="7">7 dni</option><option value="30">30 dni</option><option value="forever">Bezterminowo</option></select></label>
          </section>
        </div>
        <div className="settings-column">
          <section className="settings-group">
          ...
          <section className="model-card">
            <div className="model-card__top"><div><span className="model-kicker">Model lokalny</span><h2>NVIDIA Parakeet 0.6B v3</h2></div><span className="ready-badge"><Check size={13} />Gotowy</span></div>
            <p>Szybka transkrypcja po polsku bez wysyłania dźwięku do chmury.</p>
            <div className="model-meta"><span>Urządzenie <strong>CPU / GPU</strong></span><span>Dane <strong>tylko lokalnie</strong></span></div>
          </section>
