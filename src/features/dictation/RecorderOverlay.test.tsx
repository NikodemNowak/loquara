...
function overlay(state: DictationState) {
  ...
  const adapter = adapterStub({
    getAppSnapshot: async () => ({ dictation: state, settings: { inputDevice: null, shortcut: "Ctrl+Space", autoPaste: true, retentionDays: 30, launchOnLogin: true, startMinimized: false, activeMode: "clean", showOverlay: true, model: "parakeet", streaming: true, language: "system", modelKeepAliveSecs: 0 }, modelLoading: false }),
  ...
  return { ...view, adapter, emitState: (next: DictationState) => stateListener?.({ dictation: next, settings: { inputDevice: null, shortcut: "Ctrl+Space", autoPaste: true, retentionDays: 30, launchOnLogin: true, startMinimized: false, activeMode: "clean", showOverlay: true, model: "parakeet", streaming: true, language: "system", modelKeepAliveSecs: 0 }, modelLoading: false }), emitLevel: (level: number) => levelListener?.(level) };
}
...
describe("nakładka dyktowania", () => {
  ...
  test("potwierdzenie znika po upływie czasu i wraca do nagrywania", async () => {
    vi.useFakeTimers();
    const requestCancel = vi.fn(async () => ({ dictation: { status: "cancelling", recordingId: "a", audioPath: "a.wav" }, settings: { inputDevice: null, shortcut: "Ctrl+Space", autoPaste: true, retentionDays: 30, launchOnLogin: true, startMinimized: false, activeMode: "clean", showOverlay: true, model: "parakeet", streaming: true, language: "system", modelKeepAliveSecs: 0 }, modelLoading: false }));
  ...
  test("ponawia zachowane audio po błędzie", async () => {
    const retryTranscription = vi.fn(async () => ({ dictation: { status: "idle" }, settings: { inputDevice: null, shortcut: "Ctrl+Space", autoPaste: true, retentionDays: 30, launchOnLogin: true, startMinimized: false, activeMode: "clean", showOverlay: true, model: "parakeet", streaming: true, language: "system", modelKeepAliveSecs: 0 }, modelLoading: false }));
  ...
  test("ponawia inicjalizację po błędzie", async () => {
    const getAppSnapshot = vi.fn()
      ...
      .mockResolvedValueOnce({ dictation: { status: "idle" }, settings: { inputDevice: null, shortcut: "Ctrl+Space", autoPaste: true, retentionDays: 30, launchOnLogin: true, startMinimized: false, activeMode: "clean", showOverlay: true, model: "parakeet", streaming: true, language: "system", modelKeepAliveSecs: 0 }, modelLoading: false });
    const hideOverlay = vi.fn(async () => undefined);
  ...
  test("nie nadpisuje nowszego zdarzenia spóźnionym snapshotem startowym", async () => {
    ...
    act(() => emitState({
      ...
      settings: { inputDevice: null, shortcut: "Ctrl+Space", autoPaste: true, retentionDays: 30, launchOnLogin: true, startMinimized: false, activeMode: "clean", showOverlay: true, model: "parakeet", streaming: true, language: "system", modelKeepAliveSecs: 0 },
      modelLoading: false,
    }));
    resolveSnapshot({
